...
const DoctorDetails = ({doctor}) => {
  ...
  return (
    <div className='p-3 lg:p-5'>
      <div>
      <img src={photo} alt="" className='doctorsDetailsImg' />
      </div>
      <h2 className='text-[18px] leading-[30px] lg:text-[26px] lg:leading-9 text-headingColor font-[700]
      mt-3 lg:mt-5' >
        {name}</h2>
      <div className="mt-2 lg:mt-4 flex items-center justify-between">
        <span className='text-irishBlueColor bg-slate-300 py-1 px-2 lg:py-2 lg:px-6 text-[18px] rounded
        leading-4 lg:text-[16px] lg:leading-7 font-semibold'>
          {department}</span>
          <div className="flex items-center gap-[6px]">
            <span className="flex items-center gap-[6px] text-[14px] leading-7 lg:text-[16px] font-semibld
            text-headingColor ">
              <img src={starIcon} alt="" className='w-5'/>{avgRating}
            </span>
